...
# Initial state 
np.random.seed(67)
x = np.random.normal(size=(2,2))  # random complex vector
rho_0 = torch.tensor(
    x/np.linalg.norm(x)
...
def split_density_matrix(rho):
    x = pyq.schmidt_decomposition_numpy(rho) #x is schmidt coeff, eigenmode 1, eigenmode 2
